// Supabase persistence for the client tree.
//
// The cache hands this adapter the whole tree and the last version the backend
// is known to hold. Its job is to work out what actually changed and issue only
// those writes — clientStore rewrites the entire tree on every edit, so pushing
// it wholesale would mean rewriting every row each time somebody types.
//
// Two things make the diff sound:
//
      ...
      for (const client of next) {
        ...
        for (const location of client.locations ?? []) {
          ...
          for (const visit of location.visits ?? []) {
            ...
            const revsBefore = new Map(
              (visitBefore?.exports ?? []).map(e => [e.revision, e]),
            )
